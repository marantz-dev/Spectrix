...
def calculate_theoretical_threshold(freqs):
    """
    Calcola la curva teorica di Spectrix basata sulla somma delle Gaussiane.
    Parametri presi dalla patch "Three-Point Validation".
    """
    # Parametri della patch di test (Frequenza, Gain, Sigma)
    peaks = [
        {"freq": 100.0, "gain": -40.0, "sigma": 0.25},
        {"freq": 1000.0, "gain": -60.0, "sigma": 0.075},
        {"freq": 5000.0, "gain": -80.0, "sigma": 0.025},
    ]
    global_shift = 0.0  # Curve Shift

    # Inizializza la curva con lo shift globale
    curve = np.full_like(freqs, global_shift)

    # Evita errori di log(0)
    valid_mask = freqs > 0
    ...
    for p in peaks:
        log_mu = np.log10(p["freq"])
        sigma = p["sigma"]
        gain = p["gain"]

        # Formula Gaussiana Logaritmica (Identica al C++)
        # V_i = G_i * exp( -0.5 * ( (log(f) - log(mu))^2 / sigma^2 ) )
        exponent = -0.5 * ((log_f - log_mu) ** 2) / (sigma**2)
        gaussian_val = gain * np.exp(exponent)

        curve[valid_mask] += gaussian_val
...
def generate_spectrix_report(
    dry_file, proc1_file, proc2_file, output_file, label1, label2
):
    print(f"--- Analisi Spettrale Spectrix ---")

    for f in [dry_file, proc1_file, proc2_file]:
    ...
    phase2 = compute_phase_diff(Z_p2, Z_dry)

    # ---- SMOOTH ----
    mag_dry_s = smooth_curve(mag_dry)
    mag_p1_s = smooth_curve(mag_p1)
    mag_p2_s = smooth_curve(mag_p2)

    delta1_s = smooth_curve(delta1)
    delta2_s = smooth_curve(delta2)

    phase1_s = smooth_curve(phase1, 11)
    phase2_s = smooth_curve(phase2, 11)
    ...
    # Delta 2
    peak2_pos_idx = np.argmax(delta2_s)
    peak2_neg_idx = np.argmin(delta2_s)
    peak2_pos_freq = f[peak2_pos_idx]
    peak2_neg_freq = f[peak2_neg_idx]
    peak2_pos_val = delta2_s[peak2_pos_idx]
    peak2_neg_val = delta2_s[peak2_neg_idx]
    ...
    # Phase 2
    phase2_pos_idx = np.argmax(phase2_s)
    phase2_neg_idx = np.argmin(phase2_s)
    phase2_pos_freq = f[phase2_pos_idx]
    phase2_neg_freq = f[phase2_neg_idx]
    phase2_pos_val = phase2_s[phase2_pos_idx]
    phase2_neg_val = phase2_s[phase2_neg_idx]
    ...
    fig.suptitle("Analisi Spettrale – Confronto Segnale Dry / Processato", fontsize=16)
    ...
    ax1 = axs[0]
    ax1.semilogx(
        f, mag_dry_s, label="Dry", color="#444444", linewidth=1.6, alpha=0.7, zorder=2
    )
    # ax1.semilogx(
    #     f,
    #     calculate_theoretical_threshold(f),
    #     label=label1,
    #     color="#1f77b4",
    #     linewidth=2.0,
    #     alpha=0.75,
    ...
    # peak markers Delta2
    ax2.plot(
        peak2_pos_freq, peak2_pos_val, "o", color="#ff8800", markersize=6, zorder=5
    )
    ax2.plot(
        peak2_neg_freq, peak2_neg_val, "o", color="#ff8800", markersize=6, zorder=5
    )
    ax2.annotate(
        f"{peak2_pos_val:.1f} dB",
        (peak2_pos_freq, peak2_pos_val),
        textcoords="offset points",
        xytext=(8, 8),
        fontsize=9,
        zorder=10,
    ...
    ax2.annotate(
        f"{peak2_neg_val:.1f} dB",
        (peak2_neg_freq, peak2_neg_val),
        textcoords="offset points",
        xytext=(8, -12),
        fontsize=9,
        zorder=10,
    ...
    ax2.fill_between(f, delta2_s, 0, where=(delta2_s <= 0), color="#ff880033")
    ...
    # phase2 markers
    ax3.plot(
        phase2_pos_freq, phase2_pos_val, "o", color="#55aa33", markersize=6, zorder=5
    )
    ax3.plot(
        phase2_neg_freq, phase2_neg_val, "o", color="#55aa33", markersize=6, zorder=5
    )
    ax3.annotate(
        f"{phase2_pos_val:.2f} rad",
        (phase2_pos_freq, phase2_pos_val),
        textcoords="offset points",
        xytext=(8, 8),
        fontsize=9,
        zorder=10,
    ...
    ax3.annotate(
        f"{phase2_neg_val:.2f} rad",
        (phase2_neg_freq, phase2_neg_val),
        textcoords="offset points",
        xytext=(8, -12),
        fontsize=9,
        zorder=10,
...
if __name__ == "__main__":
    if len(sys.argv) < 6:
        print(
            "Utilizzo: python spectrix_analyzer.py <dry.wav> <processed1.wav> <processed2.wav> <output.pdf> <label1> <label2>"
        )
    else:
        generate_spectrix_report(
